...
if __name__ == '__main__':
    ...
    response = client.models.generate_content(
        model="gemini-2.0-flash", contents="User is taking a test similar to the MMSE and SAGE (Self-Administered Gerocognitive Examination) tests. Give a brief introduction to what the given test is"
    )
    ...
    qOne = questionDivider.send_message(
    ...
    questions.append(qOne.text)
    sd.caption(qOne.text)

    sd.write("Question 2a Results:")
    qTwo = client.models.generate_content(
    ...
    questions.append(qTwo.text)
    sd.caption(qTwo.text)
    ...
    q = questionDivider.send_message(
        message="Given this answer key sequence of values" + str(sampleResults["sequence_answer"]) + ", is this user-made sequence (" + str(sampleResults["sequence_qs_2"])+ ") similar to the correct answer?"
    )
    questions.append(q.text)
    sd.caption(q.text)
    ...
    q = questionDivider.send_message(
        message= "In a brief sentence, would you say the user Passed the Diagnostic or is At Risk of Cognitive Decline?"
    )
    sd.caption(q.text)
